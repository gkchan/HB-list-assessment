...
def all_odd(numbers):
    ...
    # can test whether input is int first if that is a concern
...
def foods_in_common(foods1, foods2):
    ...
    **NOTE**: for this problem, you're welcome to use any of the
    ...
    common_foods_without_duplicates = []

    for food in common_foods:
        if food not in common_foods_without_duplicates:
            common_foods_without_duplicates.append(food)

    return sorted(common_foods_without_duplicates)

    # Solution 2

    # can also find the intersection of 2 sets

    # common_foods = set(foods1) "intersection" set(foods2)
    # return sorted(list(common_foods))

    # not sure about entering symbols
...
def largest_n_items(items, n):
    ...
    for item in items:
        if item is list:
            for number in item:
                numbers_no_nesting.append(number)
        else:
            numbers_no_nesting.append(item)

    numbers_no_nesting.sort()

    if n > 0:
        largest_n_numbers = numbers_no_nesting[-n:]
